...
open("Content4.txt", 'w').close()
open("Urls4.txt", 'w').close()

def swrite(dat):
	file=open("Content4.txt","w")
	file.write(dat)
	file.close()

def twrite(dat):
	file=open("Urls4.txt","a")
	file.write(dat)
	file.close()	
...
if(conn):
	...
	data=var1.find_all("div",{"class":"widget-wrapper top_stories_ordering"})
	data[0]=str(data[0])
	swrite(data[0])
	...
	for i in range (len(urls)):
		if "http=\"" in urls[i]:
			urls[i]=urls[i].replace("href=\"","https://www.indiatoday.in")
	for i in range(0,4):
		url=urls[i][:-1]
